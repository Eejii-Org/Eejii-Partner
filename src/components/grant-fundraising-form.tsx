...
import { grantFundraisingSchema, ProjectInputs } from "@/schemas/projectSchema";
...
export const GrantFundraisingForm = ({
  ...
  grantFundraising?: ProjectType | null;
  ...
  const [startTime, setStartTime] = useState<Date>(
    new Date(grantFundraising?.startTime ?? "now"),
  );
  const [endTime, setEndTime] = useState<Date>(
    new Date(grantFundraising?.endTime ?? "now"),
  );

  const thumbnailImage =
    grantFundraising?.images?.find((img: any) => img.type == "thumbnail")
      ?.path || null;
  const coverImage =
    grantFundraising?.images?.find((img: any) => img.type == "cover")?.path ||
    null;

  const [categories, setCategories] = useState<CategoryType[]>([]);
  const projectCategories = grantFundraising?.categories.map((c) => c?.["@id"]);
  ...
  const {
    ...
    defaultValues: {
      title: grantFundraising?.title ?? "",
      description: grantFundraising?.description ?? "",
      shortDescription: grantFundraising?.shortDescription ?? "",
      categories: [],
      link: grantFundraising?.link ?? "",
      startTime: grantFundraising?.startTime ?? "",
      endTime: grantFundraising?.endTime ?? "",
      contact: {
        email: grantFundraising?.contact?.email,
        phoneNumber: grantFundraising?.contact.phoneNumber ?? "",
      },
    },
    mode: "onChange",
    resolver: zodResolver(grantFundraisingSchema),
  ...
  function handleDeleteImage(type: string) {
    const image = grantFundraising?.images.find((image) => image.type === type);
    if (image) {
      deleteImage(
        { slug: grantFundraising?.slug as string, id: image.id },
        {
          onSuccess: async () => {
            await queryClient.invalidateQueries({
              queryKey: ["project", grantFundraising?.slug],
            });
          },
        },
      );
